...
def setup_chat_rate_limiter(config: dict):
	...
	if config.setup.use_azure_api and model in AZURE_MODEL_DETAILS_MAP:
		md = AZURE_MODEL_DETAILS_MAP[model]
		return md["properties"]["rate_limit_(Requests per minute)"], md["properties"]["rate_limit_(Tokens per minute)"]
	elif model in OPENAI_MODEL_DETAILS_MAP:
		md = OPENAI_MODEL_DETAILS_MAP[model]
		return md.get("rpm", 5000), md.get("tpm", 300000)
	else:
		return 5000, 300000
...
def get_model_max_tokens(config: dict):
	...
	if config.setup.use_azure_api and model in AZURE_MODEL_DETAILS_MAP:
		...
		return model_details['properties']['max_tokens']
	elif model in OPENAI_MODEL_DETAILS_MAP:
		model_details = OPENAI_MODEL_DETAILS_MAP[model]
		# ❗️OpenAI map não tem 'properties'
		return model_details['max_tokens']
	else:
...
async def async_chat_completion_rl_inner(**kwargs):
    ...
    if use_azure_api and model in AZURE_MODEL_DETAILS_MAP:
        model_details = AZURE_MODEL_DETAILS_MAP[model]
