...
def generate_main_results_table(df_results, wandb=None, use_95_ci=True, return_all_next_step_head_n_tables=True):
    ...
    table_lines.append(r'\midrule')
    # Detect which metric column to use (reward or test_mse)
    metric_col = 'reward' if 'reward' in df_out.columns else 'test_mse'
    ...
    for method in df_out.method.unique():
        ...
        for env_name in df_out.env_name.unique():
            ...
            if method == 'relentless':
                line += r'&\bf' + f"{row[metric_col]['mean'].iloc[0]:.{sf}g}" + r'$\pm$' + f"{row[metric_col][error_metric].iloc[0]:.{sf}g}"
            else:
                line += r'&' + f"{row[metric_col]['mean'].iloc[0]:.{sf}g}" + r'$\pm$' + f"{row[metric_col][error_metric].iloc[0]:.{sf}g}"
        line += r'\\'
        table_lines.append(line)
    table_lines.append(r'\bottomrule')
    table_lines.append(r'\end{tabular}')
