...
self.re_tries)
    ...
    def generate_reflection_competition_for_generation_dict(self, generation_dict):
        ...
        elif self.env.env_name == 'Dataset-CBIO':
...
""", 'function_call': {"name": 'complete_StateDifferential_code'}})
        ...
        # Save best model code to disk
        import os
        save_dir = getattr(self.config.run, 'dynode_saved_models_folder', 'saved_models/cbio')
        os.makedirs(save_dir, exist_ok=True)

        # Save complete model code as .py file
        code_path = f'{save_dir}/best_model_{self.env.env_name}_seed{self.env.seed}.py'
        full_code = f"""# Best model generated by HDTwinGen NSDT
# Environment: {self.env.env_name}
# Seed: {self.env.seed}
# Test MSE: {test_loss:.6f}
# Val MSE: {best_program['val_loss']:.6f}

import torch
import torch.nn as nn
from typing import Tuple

{best_program['StateDifferential_code']}

# Optimized Parameters:
# {best_program.get('optimized_parameters', {})}
"""
        with open(code_path, 'w') as f:
            f.write(full_code)

        # Save metadata as JSON
        metadata_path = f'{save_dir}/best_model_{self.env.env_name}_seed{self.env.seed}_metadata.json'
        metadata = {
            'env_name': self.env.env_name,
            'seed': self.env.seed,
            'test_loss': float(test_loss),
            'val_loss': float(best_program['val_loss']),
            'train_loss': float(best_program.get('train_loss', 0)),
            'optimized_parameters': best_program.get('optimized_parameters', {}),
            'model_description': best_program.get('model_description', ''),
            'code_path': code_path,
            'loss_per_dim': best_program.get('loss_per_dim_dict', {})
        }
        with open(metadata_path, 'w') as f:
            json.dump(metadata, f, indent=2)

        self.logger.info(f'[Model code saved to] {code_path}')
        self.logger.info(f'[Model metadata saved to] {metadata_path}')

        # Find and rename the weights file matching the best model's val_loss
        best_val_loss = best_program['val_loss']
        candidate_weights_path = f'{save_dir}/candidate_weights_val{best_val_loss:.6f}.pt'
        final_weights_path = f'{save_dir}/best_model_{self.env.env_name}_seed{self.env.seed}.pt'
        if os.path.exists(candidate_weights_path):
            os.rename(candidate_weights_path, final_weights_path)
            self.logger.info(f'[Model weights saved to] {final_weights_path}')
        # Clean up other candidate weights files
        import glob
        for f in glob.glob(f'{save_dir}/candidate_weights_val*.pt'):
            try:
                os.remove(f)
            except:
                pass
...
class ZeroShot(NSDT):
    ...
    def _run(self, state=''):
        ...
        # Save complete model code as .py file
        code_path = f'{save_dir}/best_model_{self.env.env_name}_seed{self.env.seed}_{self.name}.py'
        full_code = f"""# Best model generated by HDTwinGen {self.name}
# Environment: {self.env.env_name}
# Seed: {self.env.seed}
# Test MSE: {test_loss:.6f}
# Val MSE: {best_program['val_loss']:.6f}
...
"""
        ...
        # Save metadata as JSON
        metadata_path = f'{save_dir}/best_model_{self.env.env_name}_seed{self.env.seed}_{self.name}_metadata.json'
        metadata = {
            'env_name': self.env.env_name,
            'seed': self.env.seed,
            'method': self.name,
